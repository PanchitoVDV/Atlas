...
public final class ProxyScaler extends Scaler {
    ...
    private boolean shouldScaleUpProxy() {
        ...
        if (startingProxies > 0) {
            ...
            if (thresholdMet) {
                Logger.debug("High network utilization ({}%) detected with {} starting proxies, allowing scale up", 
                        String.format("%.1f", utilization * 100), startingProxies);
            }
        }

        if (thresholdMet && canScale && !cooldownExpired) {
            Logger.debug("Proxy scale up conditions met for {} but in cooldown for {} more seconds", 
                    this.groupName, 
                    cooldownSeconds - java.time.Instant.now().getEpochSecond() + this.lastScaleUpTime.getEpochSecond());
        }
    ...
    private boolean shouldScaleDownProxy() {
        ...
        if (thresholdMet && canScale && !cooldownExpired) {
            Logger.debug("Proxy scale down conditions met for {} but in cooldown for {} more seconds", 
                    this.groupName, 
                    cooldownSeconds - java.time.Instant.now().getEpochSecond() + this.lastScaleDownTime.getEpochSecond());
        }
    ...
    private double getNetworkUtilization() {
        List<ServerInfo> autoScaledProxies = this.getAutoScaledServers();
        if (autoScaledProxies.isEmpty()) {
            return 0.0;
        }

        List<ServerInfo> runningProxies = autoScaledProxies.stream()
        ...
        for (ServerInfo proxy : autoScaledProxies) {
            totalNetworkPlayers += proxy.getOnlinePlayers();

            if (proxy.getStatus() == ServerStatus.RUNNING) {
                totalNetworkCapacity += proxy.getMaxPlayers();
            } else if (proxy.getStatus() == ServerStatus.STARTING) {
        ...
        Logger.debug("Network utilization for proxy group {}: {}% (players: {}, capacity: {})",
                this.groupName, String.format("%.1f", utilization * 100), 
                totalNetworkPlayers, totalNetworkCapacity);
