...
public final class VelocityPacketHandler extends SimpleChannelInboundHandler<Packet> implements PacketHandler {
    ...
    public VelocityPacketHandler(NetworkServerCacheManager cacheManager, VelocityServerRegistryManager registryManager, Logger logger) {
    ...
    public void handleHandshake(HandshakePacket packet) {
        ...
        if (packet.isAccepted()) {
            ...
            this.logger.error("Handshake rejected: {}", packet.getReason());
        }
    }
    ...
    public void handleServerUpdate(ServerUpdatePacket packet) {
        this.logger.debug("Server update received: {}", packet.getServerInfo().getName());

        this.cacheManager.updateServer(packet.getServerInfo());
    }
    
    @Override
    public void handleServerList(ServerListPacket packet) {
        this.logger.debug("Server list received with {} servers", packet.getServers().size());

        packet.getServers().forEach(server -> {
            this.cacheManager.updateServer(server);
            this.registryManager.handleServerAdd(server);
        });
        
        this.logger.info("Registered {} servers on startup", packet.getServers().size());
    }
    
    @Override
    public void handleServerAdd(ServerAddPacket packet) {
        this.logger.debug("Server add received: {}", packet.getServerInfo().getName());

        this.cacheManager.updateServer(packet.getServerInfo());
        this.registryManager.handleServerAdd(packet.getServerInfo());
    }
    ...
    public void handleServerInfoUpdate(ServerInfoUpdatePacket packet) {
        this.logger.debug("Server info update received: {}", packet.getServerInfo().getName());

        this.cacheManager.updateServer(packet.getServerInfo());
        this.registryManager.handleServerInfoUpdate(packet.getServerInfo());
    }
